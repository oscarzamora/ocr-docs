...
def _append_history(output_dir: Path, proposals: list[Proposal], run_time: str,
    ...
    if is_new:
        lines += [
            ...
            'Auto-generated by ocr-router. Each run appends a new section.\n\n',
            '---\n\n',
        ]

    verb = 'renamed in place' if action_mode == 'rename' else 'moved'
    lines += [f'## {run_time} — {len(proposals)} file(s) {verb}\n\n']
    dest_col = 'Final Path' if action_mode == 'rename' else 'Destination'
    lines += [
        f'| # | Original File | New Name | Category | Issuer | Doc Date | Amount | {dest_col} | Summary |\n',
        '|---|---------------|----------|----------|--------|----------|--------|-------------|---------|\n',
    ]
    ...
    for p in proposals:
        ...
        date     = _md_escape((p.metadata.get('date') or '')[:10] or '—')
        amount   = p.metadata.get('amount')
        currency = p.metadata.get('currency', '$')
        ...
        if action_mode == 'rename':
            ...
            try:
                ...
                dest = _md_escape(str(p.dest_file))
        summary  = _md_escape(_content_summary(p))

        lines.append(f'| {p.index} | {orig} | {new} | {cat} | {issuer} | {date} | {amt_str} | {dest} | {summary} |\n')
