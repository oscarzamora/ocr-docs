...
| `Credit Card Statements` | `Credit Card Statements/{issuer}/{year}` | Issuer includes owner suffix: `AMEX (OZ)`, `Bank of America (GV)`, `Citibank - AAdvantage (OZ)` |
...
Owners **OZ** (Oscar/Ozamo) and **GV** (Giovanna) are embedded in the **issuer name** as a suffix, e.g. `AMEX (OZ)`, `Bank of America (GV)`. They are **not** a separate folder level in practice.
...
- `owners: [Ozamo]` is defined but owner is embedded in the issuer string, not injected as a standalone path segment
