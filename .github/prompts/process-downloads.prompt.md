...
**Pending iteration:**
description: Process staged downloads with OCR Router using generic local-only guidance and explicit user confirmation.

| #  | Original File | Category | Issuer | New Name | Amount | Destination |
# Process staged downloads safely

Use only local OCR Router commands. Keep all examples generic and avoid personal names,
account identifiers, addresses, or employer details.

## Scope

- Input: user-provided staging folder (for example `__downloads__`)
- Output: user-provided documents root
- Config: `config/routing-config.local.yaml` (fallback `config/routing-config.yaml`)

## Workflow

1. Discover candidates (`.pdf`, `.jpg`, `.jpeg`) and present lists before acting.
2. Run dry-run first and show a review table.
3. Wait for explicit user confirmation before any move/rename.
4. Execute with `ocr-router process` only.
5. Append monthly history log entries (append-only).

## Rules

- Reuse OCR cache artifacts when present.
- Do not re-OCR PDFs that already have a text layer.
- Never silently overwrite on destination conflicts.
- Keep OCR/LLM processing local-only.
- Never send document text to cloud APIs.

## Output format

Use Markdown table columns:
